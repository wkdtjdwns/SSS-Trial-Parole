...
public class PinkGhost : MonoBehaviour
{
    ...
    private Vector3 GetPredictedPosition()
    {
        ...
        Vector3 combinedDir = (playerForward + toGhost).normalized;

        Vector3 rawTarget = playerTransform.position + combinedDir * predictionRadius;
    ...
    private Vector3 GetRandomNearbyPosition()
    {
        ...
        Debug.LogWarning("PinkGhost: 랜덤 위치 탐색 실패");
    ...
    private void TrySetDestination(Vector3 targetPosition)
    {
        ...
        {
            Debug.LogWarning("PinkGhost: 경로 실패, 플레이어 추적");
